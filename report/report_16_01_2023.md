...
Jingai Si: Implemented basic search function (that could be used in UI) to search for genes while ignoring the case, as well as a function to search for strings starting with the query "prefixSearch". Idea to improve efficiency of prefix search : trie structure. Idea to improve search functions : implement bags of words to search by prefix for each words.

...
Nicolás: During the holidays I wrote a function to retrieve gene information from the HGNC database. Initially, I tried to use qt Network to download the data, but, after some days, this proved extremely frustrating and inconvenient. To add to the frustration, the main advantage of this library, which is that it is already included in qt, was ruined by the fact that, on macOS, it requires a manual installation and configuration of OpenSSL. Eventually I decided that it would be better to use libcurl, since the implementation is much more straightforward and it is not hard to link. After the holidays I helped my teammates to install and link the library and adapted some of the code written by Clémence to work with the HGNC API function.

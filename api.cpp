...
size_t write_data(void *ptr, size_t size, size_t nmemb, QByteArray *data) {
    ...
    // qDebug() << QString(QByteArray((char*) ptr, size * nmemb));
    // std::cin.get();
    return size * nmemb;
}
...
QByteArray downloadUrl(const QUrl &url) {
    ...
    qDebug() << "Downloaded" << data.size() << "bytes in" << duration_cast<seconds>(stop - start).count() << "seconds";
    return data;
}
